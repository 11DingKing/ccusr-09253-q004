...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.RECEIVED: frozenset({State.TRIAGED, State.DECIDED}),
    State.TRIAGED: frozenset({State.DECIDED, State.ARCHIVED}),
    State.DECIDED: frozenset({State.ARCHIVED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.ARCHIVED}:
