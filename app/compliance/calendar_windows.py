...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.PLANNED: frozenset({State.OPEN, State.LOCKED}),
    State.OPEN: frozenset({State.LOCKED, State.SETTLED}),
    State.LOCKED: frozenset({State.SETTLED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.SETTLED}:
