...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.COLLECTING: frozenset({State.SUBMITTED, State.VERIFIED}),
    State.SUBMITTED: frozenset({State.VERIFIED, State.REJECTED}),
    State.VERIFIED: frozenset({State.REJECTED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.REJECTED}:
