...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.REQUESTED: frozenset({State.PREPARED, State.DOWNLOADED}),
    State.PREPARED: frozenset({State.DOWNLOADED, State.PURGED}),
    State.DOWNLOADED: frozenset({State.PURGED}),
    State.PURGED: frozenset({}),
}
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.PURGED}:
