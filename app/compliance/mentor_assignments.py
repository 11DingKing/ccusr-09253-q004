...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.PROPOSED: frozenset({State.ACCEPTED, State.REPLACED}),
    State.ACCEPTED: frozenset({State.REPLACED, State.ENDED}),
    State.REPLACED: frozenset({State.ENDED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.ENDED}:
