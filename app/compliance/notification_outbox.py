...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.PENDING: frozenset({State.LEASED, State.DELIVERED}),
    State.LEASED: frozenset({State.DELIVERED, State.DEAD}),
    State.DELIVERED: frozenset({State.DEAD}),
    State.DEAD: frozenset({}),
}
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.DEAD}:
