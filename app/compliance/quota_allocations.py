...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.RESERVED: frozenset({State.CONSUMED, State.RETURNED}),
    State.CONSUMED: frozenset({State.RETURNED, State.EXPIRED}),
    State.RETURNED: frozenset({State.EXPIRED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.EXPIRED}:
