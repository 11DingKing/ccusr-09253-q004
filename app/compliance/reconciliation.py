...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.CREATED: frozenset({State.RUNNING, State.BALANCED}),
    State.RUNNING: frozenset({State.BALANCED, State.FAILED}),
    State.BALANCED: frozenset({State.FAILED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.FAILED}:
