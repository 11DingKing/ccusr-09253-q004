...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.ACTIVE: frozenset({State.PAUSED, State.EXECUTED}),
    State.PAUSED: frozenset({State.EXECUTED, State.CANCELLED}),
    State.EXECUTED: frozenset({State.CANCELLED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.CANCELLED}:
