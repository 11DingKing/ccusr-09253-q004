...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.DRAFT: frozenset({State.PUBLISHED, State.RETIRED}),
    State.PUBLISHED: frozenset({State.RETIRED, State.RESTORED}),
    State.RETIRED: frozenset({State.RESTORED}),
    State.RESTORED: frozenset({}),
}
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.RESTORED}:
