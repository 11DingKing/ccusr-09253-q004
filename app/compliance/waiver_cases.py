...
ALLOWED_TRANSITIONS: Mapping[State, frozenset[State]] = {
    State.OPENED: frozenset({State.REVIEWING, State.APPROVED}),
    State.REVIEWING: frozenset({State.APPROVED, State.DECLINED}),
    State.APPROVED: frozenset({State.DECLINED}),
...
def transition(record: Record, target: State, actor_id: str, now: datetime, reason: str) -> Record:
    ...
    if record.expired(instant) and target not in {State.DECLINED}:
